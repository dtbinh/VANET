...
import simulation.solutions.custom.ACO_MWAC.AntAssistant.Routing_table;
...
public class ACO_MWACNeighboorList {
	...
	//******************************************************************************************************************************************************************
	/**
	 * renvoi la liste des agents voisins impliqués dans le routage (selon le role de l'agent source)
	 * @return the set of agent identifiers
	 */
	public int [] listOfRoutingAgent(int Role){
		return this.listOfRoutingAgent(0, Role, new int [0]);
	}

	public int [] listOfRoutingAgent(int Id, int Role, int [] memory)
	{
		TripletIdRoleGroup t;
		ArrayList<Integer> res = new ArrayList<Integer>() ;
		ListIterator<TripletIdRoleGroup> iter = neighboorList.listIterator();
		while(iter.hasNext())
		{
			t=iter.next();
			switch(Role){
			case MWACAgent.roleNOTHING : 		break;
			case MWACAgent.roleSIMPLEMEMBER:    if ((t.role == MWACAgent.roleREPRESENTATIVE)) 					res.add(t.id); break;
			case MWACAgent.roleLINK:			if ((t.role == MWACAgent.roleREPRESENTATIVE)) 					res.add(t.id); break;
			case MWACAgent.roleREPRESENTATIVE: 	if ((t.role == MWACAgent.roleLINK) && NotPrevent(Id,t,memory)) 	res.add(t.id); else System.out.println("prevention"); break;
			}
		}
		
		int [] Agent_list = new int [res.size()]; 
		for (int i=0; i<res.size(); i++) 
			Agent_list [i]= res.get(i);	
		return Agent_list;
	}
	
	//s'il existe au moins un element qui peut etre joint (prevention des retours arriere)
	public boolean NotPrevent(int Id, TripletIdRoleGroup t, int [] route){
		for (int i=0; i<t.groups.length; i++){
			if ( (Id != t.groups[i]) && (Ant_Route_Assistant.contains(route, t.groups[i])==false)) return true;
		}
		return false;
	}
	//******************************************************************************************************************************************************************
