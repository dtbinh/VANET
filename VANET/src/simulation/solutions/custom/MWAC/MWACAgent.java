...
public class MWACAgent extends Agent implements ObjectAbleToSendMessageInterface{
	...
	public void run()
	{
		...
		while(!isKilling() && !isStopping())
		{
			...
			while( this.receivedMessageQueue.isEmpty() && (!interaction.lostRepresentativeRole) && (!interaction.reinitializationOfTheRole)) 
			{
				...
				if (this.mustUsurpsWorkstationID || this.mustUsurpsNeighboorID )
				{
					if( !this.isUsurper)
					{
						if (this.mustUsurpsWorkstationID) 
						{
							...
							this.isUsurper=true;
							this.setUserId(this.WORKSTATION_ID);
	...
	public void Nacer(String str,int i,int j)
	{
		System.out.println("\n\n\n"+str+"("+i+","+j+")\n\n");
	}
				...
				if(this.role==MWACAgent.roleREPRESENTATIVE)
				{
					...
					for(int i=0;i<suspectedGroups.length;i++)
					{
						if(!this.organizationalIncoherenceManager.isSuspectedGroup(suspectedGroups[i]))
						{
							...
							super.sendFrame(new MWACFrame(getUserId(),MWACFrame.BROADCAST_LINK,new MWACMessage_TTLRouteRequest(getUserId(),suspectedGroups[i],this.messageToTransmitQueue.getNextIdRequest(),(byte) MWACAgent.MAX_HOP_RESEARCH_ORGANIZATIONAL_INCOHERENCE)));
						}						
					}
				}
				else
	...
	{
		if(MWACAgent.DEBUG) debug(">>> Envoie par d'un message d'INTRODUCTION      <"+getUserId()+","+MWACAgent.roleToString(this.role)+","+this.getGroup()+">");
