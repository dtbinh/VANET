...
public class DSRSocialAlreadyProcessedRouteRequestManager {

	private static int ms_MAX_ROUTE_REQUEST_DELAY = 10000;

	private LinkedList<AlreadyProcessedRequestItem> list;

	public DSRSocialAlreadyProcessedRouteRequestManager()
	{
	...
	public synchronized void verifyAges()
	{
		...
		{
			item=iter.next();
			if (now.differenceToMS(item.date)>DSRSocialAlreadyProcessedRouteRequestManager.ms_MAX_ROUTE_REQUEST_DELAY) iter.remove();
		}
	}
