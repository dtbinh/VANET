...
public class MWACNeighboorList
{
	...
	public String toHTML()
	{
		...
		{
			triplet=iter.next();
			res+="<TR><TD>"+triplet.id+"</TD><TD>"+AntMWACAgent.roleToString(triplet.role)+"</TD><TD>"+MWACGroupAssistant.groupsToString(triplet.groups)+"</TD></TR>";
		}
		res+="</TABLE>";
		return res;
	}
	...
	public int getLinkToRepresentant(int representativeId)
	{
		...
		{
			triplet=iter.next();
			if(triplet.role==AntMWACAgent.roleLINK) trouve=triplet.inGroup(representativeId);
		}

		if(trouve)
		{
			// on le deplace pour qu'il ne soit pas utilis� en priorit� au prochain coup
			iter.remove();
	...
	public int getNbRepresentative()
	{
		int nb=0;
		ListIterator<TripletIdRoleGroup> iter = neighboorList.listIterator();
		while(iter.hasNext()) if(iter.next().role==AntMWACAgent.roleREPRESENTATIVE) nb++;
		return nb;
	}
	...
	public ArrayList<Integer> getRepresentativeIdentifiers()
	{
		...
		{
			t=iter.next();
			if(t.role==AntMWACAgent.roleREPRESENTATIVE) res.add(t.id);
		}
		return res;
	}
	...
	public Vector<Integer> suspiciousNeighboorsGroups(int[] ownerGroupIdentifier)
	{
		...
		{
			triplet=iter.next();
			if (triplet.role==AntMWACAgent.roleREPRESENTATIVE)
			{
				lst.removeElement(triplet.id);
				iter.remove();
			}
			else if (triplet.role==AntMWACAgent.roleLINK)
			{
				t = triplet.groups;
		...
		if(!ok.isEmpty())
		{
			...
			{
				triplet=iter.next();
				if (triplet.role==AntMWACAgent.roleLINK)
				{
					t = triplet.groups;
