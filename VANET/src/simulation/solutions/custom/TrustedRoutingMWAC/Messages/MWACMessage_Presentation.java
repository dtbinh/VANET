...
import simulation.solutions.custom.TrustedRoutingMWAC.TrustedRoutingMWACAgent;
...
public class MWACMessage_Presentation extends MWACMessage {
	...
	public void addGroup(int id) {
		...
		for (i = 0; i < this.group.length; i++)
			newGroup[i] = this.group[i];
		newGroup[i] = id;
		this.group = newGroup;
	}

	public byte[] toByteSequence() {
		ByteBuffer buf = ByteBuffer.allocate(1 + 4 * this.group.length).put(
				role);
		for (int i = 0; i < this.group.length; i++)
			buf.putInt(this.group[i]);
	...
	public String toString() {
		return "Presentation message instancied by " + this.getSender()
				+ " : [role=" + TrustedRoutingMWACAgent.roleToString(this.role)
				+ ",group=" + MWACGroupAssistant.groupsToString(this.group)	+ "]";
	}
}
