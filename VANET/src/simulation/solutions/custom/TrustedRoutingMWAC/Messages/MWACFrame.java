...
public class MWACFrame extends Frame {
	...
	public static String receiverIdToString(int id) {
		switch (id) {
		case BROADCAST_LINK:
			return "ALL_CLOSE_LINK_AGENT";
		case BROADCAST_REPRESENTATIVE:
			return "ALL_CLOSE_REPRESENTATIVE_AGENT";
		default:
			return "" + id;
		}
	}
	...
	public static MWACFrame createFrameFromBytes(byte[] msg) {
		...
		for (int i = 0; i < msg.length - 8; i++)
			data[i] = buf.get();
		System.out.println("*IN CREATE FRAME "
				+ MWACMessage.debugByteArray(msg) + " -> "
				+ MWACMessage.debugByteArray(data) + "  sender=" + sender
				+ "  receiver=" + receiver + "  interpretation="
				+ (new MWACFrame(sender, receiver, data)).toString());
		...
		if (receiver == -1)
			...
			try {
				return "Frame from "
						...
						+ super.getReceiver()
						+ ". Message is  "
						+ MWACMessage.createMessage(super.getData())
								.toString();
			} catch (Exception e) {
				System.out.println("SENDER=" + super.getSender() + "  DEST="
						+ super.getReceiver());
