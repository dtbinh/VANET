...
public class MWACAlreadyProcessedRouteRequestManager {
	...
	public synchronized void verifyAges() {
		...
		ListIterator<AlreadyProcessedRequestItem> iter = this.list
				.listIterator();
		aDate now = new aDate();
		while (iter.hasNext()) {
			item = iter.next();
			if (now.differenceToMS(item.date) > MWACAlreadyProcessedRouteRequestManager.ms_MAX_ROUTE_REQUEST_DELAY)
				iter.remove();
		}
	}
	...
	public synchronized boolean isAlreadyProcessed(int sender, short request) {
		...
		while (iter.hasNext()) {
			item = iter.next();
			if (!finded
					&& ((item.sender == sender) && (item.idRouteRequest == request)))
				finded = true;
			if (now.differenceToMS(item.date) > MWACAlreadyProcessedRouteRequestManager.ms_MAX_ROUTE_REQUEST_DELAY)
				iter.remove();
		}
	...
	public String toHTML() {
		String res = "<B>Table of already processed request</B> ("
				+ this.list.size() + ")<BR>";
		res += "<TABLE border=1>";
	...
	private class AlreadyProcessedRequestItem {
		...
		public String toHTML() {
			return "<TR><TD>"
					+ aDate.msToHHMMSSCCC(this.date.getTimeInMillis())
					+ "</TD><TD>" + this.sender + "</TD><TD>"
					+ this.idRouteRequest + "</TD></TR>";
		}
	}
