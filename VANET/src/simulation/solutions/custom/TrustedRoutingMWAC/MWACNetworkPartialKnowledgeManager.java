...
/**
 * Network partial knowledge manager Manage the route fragment and the
 * agent/group well known association
 * 
...
public class MWACNetworkPartialKnowledgeManager {
	...
	public int[] getDataMessageRoute(int dest) {
		...
		int[] route = null;
		int repr = group(dest);
		if (repr == MWACNetworkPartialKnowledgeManager.UNDEFINED_ID)
			return null;
	...
	public int getRouteRequestAssociatedReceiver(int routeRequestId) {
		...
		ListIterator<RouteRequestAndReceiverAssociationItem> iter = routeRequestAndReceiverAssociation
				.listIterator();
	...
	public void addRouteRequestAndReceiverAssociation(int routeRequestId,
			int senderOfTheMessage) {
		this.routeRequestAndReceiverAssociation
				.add(new RouteRequestAndReceiverAssociationItem(routeRequestId,
						senderOfTheMessage));
	}
	...
	public void addIdGroupAssociation(int idRepresentative, int member) {
		...
		ListIterator<GroupAssociation> iter = this.idGroupAssociation
				.listIterator();
		...
		if (!find)
			this.idGroupAssociation.add(new GroupAssociation(idRepresentative,
					member));
	...
	public String toHTML() {
		...
		res = "<B>Group association</B> (" + this.idGroupAssociation.size()
				+ ")<BR>";
		res += "<TABLE border=1>";
		...
		for (int i = 0; i < this.routes.size(); i++)
			res += "<TR><TD>"
					+ MWACRouteAssistant.routeToString(this.routes.get(i))
					+ "</TD></TR>";
		res += "</TABLE>";

		res += "<BR>";

		res += "<B>Association(Route request/sender)</B> ("
				+ this.routes.size() + ")<BR>";
		res += "<TABLE border=1>";
	...
	private class GroupAssociation {
		...
		public void add(int member) {
			this.members = MWACGroupAssistant.cloneGroupArray(this.members,
		...
		public String toHTML() {
			return "<TR><TD>" + this.representative + "</TD><TD>"
	...
	public class RouteRequestAndReceiverAssociationItem {
		...
		public RouteRequestAndReceiverAssociationItem(int routeRequest,
				int sender) {
		...
		public String toHTML() {
			return "<TR><TD>" + this.routeRequest + "</TD><TD>" + this.sender
					+ "</TD><TR>";
		}
	}
