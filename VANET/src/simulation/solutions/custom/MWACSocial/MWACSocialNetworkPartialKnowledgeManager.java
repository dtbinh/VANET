...
public class MWACSocialNetworkPartialKnowledgeManager {
	...
	public MWACSocialNetworkPartialKnowledgeManager()
	{
	...
	public int[] getDataMessageRoute(int dest)
	{
		...
		int[] route=null;
		int repr=group(dest);
		if (repr==MWACSocialNetworkPartialKnowledgeManager.UNDEFINED_ID) return null;
		...
		{
			route=iter.next();
			find=MWACSocialRouteAssistant.contains(route, repr);
		}

		if(find) 
		{
			System.out.println("!J'ai trouvé une route "+MWACSocialRouteAssistant.integerArrayToString(route));
			find=false;
			int i=0;
			...
			route= MWACSocialRouteAssistant.subRoute(route,i);
			System.out.println("Je fais la nouvelle route");
			System.out.println("La route est devenur "+MWACSocialRouteAssistant.integerArrayToString(route));
			System.out.flush();
			return route;
		}
		else
	...
	public int getRouteRequestAssociatedReceiver(int routeRequestId)
	{
		...
		{
			item=iter.next();
			if(item.routeRequest==routeRequestId) return item.sender;
		}
		iter.remove();
		return MWACSocialNetworkPartialKnowledgeManager.UNDEFINED_ID;
	}
	...
	public String toHTML()
	{
		...
		res+="<TABLE border=1>";
		for(int i=0;i<this.routes.size();i++) res+="<TR><TD>"+MWACSocialRouteAssistant.integerArrayToString(this.routes.get(i))+"</TD></TR>";
		res+="</TABLE>";
	...
	private class GroupAssociation
	{
		...
		{
			this.members=MWACSocialGroupAssistant.cloneGroupArray(this.members,1+this.members.length);
		...
		public String toHTML()
		{
			return "<TR><TD>"+this.representative+"</TD><TD>"+MWACSocialGroupAssistant.groupsToString(this.members)+"</TD></TR>";
		}
	}
