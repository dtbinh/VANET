 ...
 */
public class FrenchToEnglishTranslator extends Agent implements ObjectAbleToSendMessageInterface{

	private final static String PREY_SPRITE_FILENAME = "D:\\TestMASH\\Sprites\\translator.png";
	 ...
	 */
	public FrenchToEnglishTranslator(MAS mas, Integer id, Float energy,  Integer range) {
		super(mas, id, range);
		try {
			this.view=new ImageFileBasedObjectView(FrenchToEnglishTranslator.PREY_SPRITE_FILENAME);
		} catch (IOException e) {
	...
	public synchronized void receivedFrame(ClockUsersAndTranslatorsFrame frame)
	{
		if(frame.getReceiver()==this.getUserId() || frame.getReceiver()==Frame.BROADCAST)
		{
			if(frame.getClockUsersAndTranslatorsMessageInFrame().getStringMessage().equals("Quelle heure est-il?"))
				this.sendFrame( new ClockUsersAndTranslatorsFrame(this.getUserId(),frame.getSender(),new ClockUsersAndTranslatorsMessage(this.getUserId(),frame.getSender(),"What time is it?")));
				}
	}
