...
public class PredatorAgent extends Agent implements ObjectAbleToSendMessageInterface{
	...
	private final static String PREDATOR_SPRITE_FILENAME = "D:\\TestMASH\\Sprites\\PREDATOR.PNG";
	...
	public PredatorAgent(MAS mas, Integer id, Float energy,  Integer range) {
		super(mas, id, range);
		try {
			this.setNativeView(new ImageFileBasedObjectView(PredatorAgent.PREDATOR_SPRITE_FILENAME));
		} catch (IOException e) {
	...
	public void run()
	{
		...
		System.out.println("Démarrage de l'entité PredatorAgent "+this.getUserId());
		...
		while(!isKilling() && !isStopping())
		{
			...
			LinkedList<SimulatedObject> neighboors = getPerceivedAgent(PredatorAgent.DEFAULT_VISION_RADIUS);
			...
			while(iter.hasNext())
			{
				...
				if(item.getObject().getUserId()==1)
				{
					this.motionManagement.setDirection(item.getPosition());
					finded=true;
				}
			}
			...
			if(!finded) 
				if(neighboors.isEmpty())
					...
					this.motionManagement.setDirection(neighboors.getFirst().getPosition());
