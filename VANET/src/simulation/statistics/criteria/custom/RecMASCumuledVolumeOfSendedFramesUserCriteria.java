 ...
 */ 
public class RecMASCumuledVolumeOfSendedFramesUserCriteria extends UserCriterion{
	...
	public String getName() {
		return "ResMAS Cumuled volume of sended frames.";
	}
		...
		while(iter.hasNext())
		{
			...
			if (id ==-1 || id==evt.getRaiser().getId())
			{
				...
				if (evt instanceof SendedFrameEvent) 
				{
					...
					if (recMsg==null ) 
						cumul+=((SendedFrameEvent) evt).getFrame().getVolume();
					else 
					{
						
						if (recMsg.getType()==RecMASMessage.msgINTERN_SYSTEM_MESSAGE_TRANSPORT)
						{
							if(((RecMASMessage_msgInternSystemMessageTransport) recMsg).getLabel()<50) cumul+=((SendedFrameEvent) evt).getFrame().getVolume();
						}
						else if (recMsg.getType()==RecMASMessage.msgEXTERN_SYSTEM_MESSAGE_TRANSPORT)
						{
							if(((RecMASMessage_msgExternSystemMessageTransport) recMsg).getLabel()<50) cumul+=((SendedFrameEvent) evt).getFrame().getVolume();
						}
						else
							cumul+=((SendedFrameEvent) evt).getFrame().getVolume();
