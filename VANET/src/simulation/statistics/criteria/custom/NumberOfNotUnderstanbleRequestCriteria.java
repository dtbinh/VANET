 ...
 */ 
public class NumberOfNotUnderstanbleRequestCriteria extends UserCriterion{
	...
	public String getDescription() {
		return "Allows to see the number of not understanble request";
	}
	...
	public String getName() {
		return "Numbers of not undestranble requests.";
	}
	...
	public AbstractDataBlock getDataBlock(SimulationJournal journal, int id) {
		...
		DataBlock data = new DataBlock("Number of not understanble requests","Time",DataBlock.TIME_IN_MS,"Number of request",DataBlock.SIMPLE_VALUE_INT);
		data.addPoint(0,0);
		...
		if (id==-1) 
			eventList=journal.extractAllEvents();
		else
			eventList=journal.extractAllEvents(id);
		...
		while(iter.hasNext())
		{
			...
			{
				if (evt instanceof NotUnderstanbleRequestEvent) cumul++;
