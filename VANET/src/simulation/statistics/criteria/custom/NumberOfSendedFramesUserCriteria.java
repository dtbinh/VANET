 ...
 */ 
public class NumberOfSendedFramesUserCriteria extends UserCriterion{
	...
	public AbstractDataBlock getDataBlock(SimulationJournal journal, int id) {
		...
		while(iter.hasNext())
		{
			...
			{
				if (evt instanceof SendedFrameEvent) cumul++;
				
				if (evt.getDate().getTimeInMillis()-lastDate>this.SLOT_TIME) 
				{
					long dateOfEvent=evt.getDate().getTimeInMillis();
					while((dateOfEvent-lastDate-this.SLOT_TIME)>this.SLOT_TIME){lastDate+=this.SLOT_TIME;data.addPoint(lastDate,cumul);}
