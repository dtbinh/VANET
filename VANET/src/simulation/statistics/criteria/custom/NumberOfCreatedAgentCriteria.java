...
import simulation.events.system.AgentCreationEvent;
 ...
 */ 
public class NumberOfCreatedAgentCriteria extends UserCriterion{
	...
	public String getDescription() {
		return "Allows to see the number of created agent in the MAS during the simulation";
	}
	...
	public String getName() {
		return "Numbers of created agents.";
	}
	...
	public AbstractDataBlock getDataBlock(SimulationJournal journal, int id) {
		...
		DataBlock data = new DataBlock("Number of created agents","Time",DataBlock.TIME_IN_MS,"Number of frames",DataBlock.SIMPLE_VALUE_INT);
		data.addPoint(0,0);
		...
		while(iter.hasNext())
		{
			...
			{
				if (evt instanceof ObjectCreationEvent) 
					{
					cumul++;
					res+=((ObjectCreationEvent) evt).getSystemId()+"\t";
					}
		...
		System.err.println("Created agent criteria : "+cumul+" agents\n"+res);
