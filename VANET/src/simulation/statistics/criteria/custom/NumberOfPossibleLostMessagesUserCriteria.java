 ...
 */ 
public class NumberOfPossibleLostMessagesUserCriteria extends UserCriterion{
	...
	public String getDescription() {
		return "Allows to see the number of possible lost messages in the MAS or sended by an Agent/Object";
	}
	...
	public String getName() {
		return "Numbers of possible lost messages.";
	}
	...
	public AbstractDataBlock getDataBlock(SimulationJournal journal, int id) {
		...
		DataBlock data = new DataBlock("Number of possible lost messages","Time",DataBlock.TIME_IN_MS,"Number of messages",DataBlock.SIMPLE_VALUE_INT);
			data.addPoint(0,0);
		this.SLOT_TIME = journal.getLastEventDate().getTimeInMillis()/1000;
		if (this.SLOT_TIME==0) this.SLOT_TIME=1;
		...
		if (id==-1)
		{
			...
			while(iter.hasNext())
			{
				...
				if (evt instanceof SendedMessageEvent) cumul++;
				else if (evt instanceof ReceivedMessageEvent) cumul--;
		...
		else
		{
			...
			while(iter.hasNext())
			{
				...
				if (id==evt.getRaiser().getId() || ((evt instanceof ReceivedMessageEvent) && ( ((ReceivedMessageEvent) evt).getMessage().getSender()==id )))
				{
					if (evt instanceof SendedMessageEvent) last++;
					else if (evt instanceof ReceivedMessageEvent) last--;
