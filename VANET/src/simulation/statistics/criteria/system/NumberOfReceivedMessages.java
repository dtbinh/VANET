 ...
 */ 
public class NumberOfReceivedMessages extends SystemCriterion{
	...
	public String getDescription() {
		return "Allows to see the number of received messages in the MAS or by an Agent/Object";
	}
	...
	public String getName() {
		return "Numbers of received messages";
	}
	...
	public AbstractDataBlock getDataBlock(SystemBasicMeasurement measurementSystem, int id) {
		...
		DataBlock data = new DataBlock("Number of received messages","Time",DataBlock.TIME_IN_MS,"Number of messages",DataBlock.SIMPLE_VALUE_INT);
		data.addPoint(0,0);
		...
		{	
			...
			while(iterLst.hasNext())
			{
				...
				int i=0;
				// For each measures...
				while(iterItem.hasNext()) {tab[i++]+=iterItem.next().nbReceivedMessage;}
			}
		...
		else
		{
			...
			int i=0,n=0;
			// For each measures...
			while(iterItem.hasNext()) {data.addPoint(measurementSystem.getMeasureTime(i),n=iterItem.next().nbReceivedMessage); i++;}
			data.addPoint(measurementSystem.getTime(),n);
