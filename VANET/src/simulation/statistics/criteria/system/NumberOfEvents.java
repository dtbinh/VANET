 ...
 */ 
public class NumberOfEvents extends SystemCriterion{
	...
	public String getDescription() {
		return "Allow to see the number of events associated to the MAS/Agent/Object";
	}
	...
	public String getName() {
		return "Number of events system";
	}
	...
	public AbstractDataBlock getDataBlock(SystemBasicMeasurement measurementSystem, int id) {
		...
		DataBlock data = new DataBlock("Number of events","Time",DataBlock.TIME_IN_MS,"Number of event",DataBlock.SIMPLE_VALUE_INT);
		data.addPoint(0,0);
		...
		{	
			...
			while(iterLst.hasNext())
			{
				...
				int i=0;
				// For each measures...
				while(iterItem.hasNext()) {tab[i++]+=iterItem.next().nbEvent;}
			}
		...
		else
		{
			...
			int i=0,nbEvents=0;
			// For each measures...
			while(iterItem.hasNext()) {data.addPoint(measurementSystem.getMeasureTime(i),nbEvents=iterItem.next().nbEvent); i++;}
			data.addPoint(measurementSystem.getTime(),nbEvents);
