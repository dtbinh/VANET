 ...
 */ 
public class NumberOfSendedFrames extends SystemCriterion{
	...
	public String getDescription() {
		return "Allows to see the number of sended frames in the MAS or by an Agent/Object";
	}
	...
	public String getName() {
		return "Numbers of sended frames";
	}
	...
	public AbstractDataBlock getDataBlock(SystemBasicMeasurement measurementSystem, int id) {
		...
		{	
			...
			while(iterLst.hasNext())
			{
				...
				int i=0;
				// For each measures...
				while(iterItem.hasNext()) {tab[i++]+=iterItem.next().nbSendedFrame;}
			}
		...
		else
		{
			...
			int i=0,n=0;
			// For each measures...
			while(iterItem.hasNext()) {data.addPoint(measurementSystem.getMeasureTime(i),n=iterItem.next().nbSendedFrame); i++;}
