 ...
 */ 
public class CumuledVolumeOfReceivedMessages extends SystemCriterion{
	...
	public String getDescription() {
		return "Allows to see the cumuled volume of all received messages in the MAS or by an Agent/Object";
	}
	...
	public String getName() {
		return "Cumuled volume of received messages";
	}
	...
	public AbstractDataBlock getDataBlock(SystemBasicMeasurement measurementSystem, int id) {
		...
		DataBlock data = new DataBlock("Cumuled volume of received messages","Time",DataBlock.TIME_IN_MS,"Cumuled volume",DataBlock.SIMPLE_VALUE_INT);
		data.addPoint(0,0);
		...
		{	
			...
			while(iterLst.hasNext())
			{
				...
				int i=0;
				// For each measures...
				while(iterItem.hasNext()) {tab[i++]+=iterItem.next().volumeReceivedMessage;}
			}
		...
		else
		{
			...
			int i=0;long v=0;
			// For each measures...
			while(iterItem.hasNext()) {data.addPoint(measurementSystem.getMeasureTime(i),v=iterItem.next().volumeReceivedMessage); i++;}
			data.addPoint(measurementSystem.getTime(),v);
		}
		return data;
	}
