 ...
 */ 
public class CumuledVolumeOfSendedFrames extends SystemCriterion{
	...
	public String getName() {
		return "Cumuled volume of sended frames";
	}
	...
	public AbstractDataBlock getDataBlock(SystemBasicMeasurement measurementSystem, int id) {
		...
		{	
			...
			while(iterLst.hasNext())
			{
				...
				int i=0;
				// For each measures...
				while(iterItem.hasNext() && i<dim) {tab[i++]+=iterItem.next().volumeSendedFrame;}
			}
		...
		else
		{
			...
			int i=0; long l=0;
			// For each measures...
			while(iterItem.hasNext()) {data.addPoint(measurementSystem.getMeasureTime(i),l=iterItem.next().volumeSendedFrame); i++;}
