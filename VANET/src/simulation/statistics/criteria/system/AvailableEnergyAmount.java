 ...
 */ 
public class AvailableEnergyAmount extends SystemCriterion{
	...
	public String getDescription() {
		return "Allows to see the amount of available energy  in the MAS/Agent/Object";
	}
	...
	public AbstractDataBlock getDataBlock(SystemBasicMeasurement measurementSystem, int id) {
		...
		{	
			...
			while(iterLst.hasNext())
			{
				...
				int i=0;
				// For each measures...
				while(iterItem.hasNext()) {tab[i++]+=iterItem.next().energy;}
			}
		...
		else
		{
			...
			int i=0; float energy=0;
			// For each measures...
			while(iterItem.hasNext()) {data.addPoint(measurementSystem.getMeasureTime(i),energy=iterItem.next().energy); i++;}
			data.addPoint(measurementSystem.getTime(),energy);
		}
		return data;
	}
