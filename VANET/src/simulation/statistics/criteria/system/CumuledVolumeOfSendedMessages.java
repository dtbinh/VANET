 ...
 */ 
public class CumuledVolumeOfSendedMessages extends SystemCriterion{
	...
	public AbstractDataBlock getDataBlock(SystemBasicMeasurement measurementSystem, int id) {
		...
		{	
			...
			while(iterLst.hasNext())
			{
				...
				int i=0;
				// For each measures...
				while(iterItem.hasNext()) {tab[i++]+=iterItem.next().volumeSendedMessage;}
			}
		...
		else
		{
			...
			int i=0; long l=0;
			// For each measures...
			while(iterItem.hasNext()) {data.addPoint(measurementSystem.getMeasureTime(i),l=iterItem.next().volumeSendedMessage); i++;}
