 ...
 */ 
public class NumberOfReceivedFrames extends SystemCriterion{
	...
	public String getDescription() {
		return "Allows to see the number of received frames in the MAS or by an Agent/Object";
	}
	...
	public String getName() {
		return "Numbers of received frames";
	}
	...
	public AbstractDataBlock getDataBlock(SystemBasicMeasurement measurementSystem, int id) {
		...
		DataBlock data = new DataBlock("Number of received frames","Time",DataBlock.TIME_IN_MS,"Number of frames",DataBlock.SIMPLE_VALUE_INT);
		data.addPoint(0,0);
		...
		{	
			...
			while(iterLst.hasNext())
			{
				...
				int i=0;
				// For each measures...
				while(iterItem.hasNext()) {tab[i++]+=iterItem.next().nbReceivedFrame;}
			}
		...
		else
		{
			...
			int i=0;int n=0;
			// For each measures...
			while(iterItem.hasNext()) {data.addPoint(measurementSystem.getMeasureTime(i),n=iterItem.next().nbReceivedFrame); i++;}
